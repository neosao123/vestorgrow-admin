...
export default function Banner() {
  ...
  async function getBanner() {
    ...
    try {
      let response = await bannerServ.listAll(activity);
      if (response.data) {
  ...
  const deleteHandler = async (dataId) => {
    try {
      if (window.confirm("Are you sure you want to delete this record?")) {
        let response = await bannerServ.deleteRecord(dataId);
        if (response) {
  ...
  const activeToggleHandler = async (course) => {
    try {
      ...
      let response = await bannerServ.activeToggle(obj);
      if (response.message) {
        getBanner();
      }
    } catch (err) {
  ...
  const actionsButtons = (row) => {
    return (
      <div className="updates_icon p-0">
        <Link to={`/create_banner/${row._id}`} className="editIconCustom fa-sharp me-1 fa-solid fa-pen-to-square" >
          {/* <img
  ...
  return (
    ...
    >
      <LinkName
        heading="Publish/Manage - Banner"
        image="/assets/images/icons/publish.svg"
        span="Create a Banner"
        link="/create_banner"
      />
