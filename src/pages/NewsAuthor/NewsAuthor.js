...
function NewsAuthor() {
    ...
    async function getCategoryList() {
        ...
        try {
            let response = await authorServ.listAllCategory(activity);
            if (response) {
    ...
    const deleteHandler = async (categoryId, i) => {
        try {
            if (window.confirm("Are you sure you want to delete this record?")) {
                let response = await authorServ.deleteCategoryRecord(categoryId);
                if (response) {
    ...
    const activeToggleHandler = async (category) => {
        ...
        try {
            const res = await authorServ.editCategoryRecord(obj);
    ...
    const renderButtons = (row) => {
        return (
            <div className="updates_icon p-0">
                <Link to={`/create_newsauthor/${row._id}`} className="editIconCustom fa-sharp me-1 fa-solid fa-pen-to-square" >
    ...
    return (
        ...
        <label className="press me-2">
          ...
          <span className="lbl"></span>
            ...
            <LinkName
                heading="Publish/Manage - News Author"
                image="/assets/images/icons/publish.svg"
                span="Add Author"
                link="/create_newsauthor"
            />
