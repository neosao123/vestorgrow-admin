...
export default function NewsAuthorForm() {
    ...
    const getSingleCategoryData = async () => {
        try {
            let response = await authorServ.getCategoryDetails(params?.id);
            if (response) {
                setValue({
    ...
    const onSubmit = (values, { resetForm }) => {
        ...
        if (values._id) {
            authorServ.editCategoryRecord(values).then((res) => {
                if (res.err) {
                    toast.error(res.err);
                } else {
                    router("/newsauthor");
            ...
            authorServ.addCategoryRecord(obj).then((res) => {
                if (res.err) {
                    toast.error(res.err);
                }
                else if (res.message) {
                ...
                <div className="users_bottom_part">
                    <div className="total_updates_top ActiveLinks">
                        ...
                        <div className="walletAddressHead accountChangeHead ">
                            <h5 className="m-0">Add a Author Form</h5>
                        </div>
