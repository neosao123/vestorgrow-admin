...
function NewsCategory() {
    ...
    const NewscategoryServ = new NewsCategoryService();
    ...
    async function getCategoryList() {
        ...
        try {
            let response = await NewscategoryServ.listAllCategory(activity);
            if (response) {
    ...
    const deleteHandler = async (categoryId, i) => {
        try {
            if (window.confirm("Are you sure you want to delete this record?")) {
                let response = await NewscategoryServ.deleteCategoryRecord(categoryId);
                if (response) {
    ...
    const activeToggleHandler = async (category) => {
        ...
        try {
            const res = await NewscategoryServ.editCategoryRecord(obj);
    ...
    const renderButtons = (row) => {
        return (
            <div className="updates_icon p-0">
                <Link to={`/create_newscategory/${row._id}`} className="editIconCustom fa-sharp me-1 fa-solid fa-pen-to-square" >
    ...
    return (
        ...
        <label className="press me-2">
          ...
          <span className="lbl"></span>
            ...
            <LinkName
                heading="Publish/Manage - News Category"
                image="/assets/images/icons/publish.svg"
                span="Add News Category"
                link="/create_newscategory"
            />
