...
export default function ContributorForm() {
  ...
  const contribServ = new ContributerService();
  ...
  useEffect(() => {
    if (params?.id) {
      getContributerDetail(params.id);
    }
  }, []);

  const getContributerDetail = async (id) => {
    try {
      let response = await contribServ.getDetails(id);
      if (response) {
        setValue({
  ...
  const ValidateSchema = Yup.object({
    ...
    job_role: Yup.string().required("Job role is a required field"),
  ...
  const onSubmit = async (values) => {
    ...
    formData.set("job_role", values.job_role);
    formData.set("description", values.description);
    formData.set("social_links", JSON.stringify(values.social_links));
    try {
      let response;
      if (values._id) {
        response = await contribServ.updateRecord(formData);
      } else {
        response = await contribServ.addrecord(formData);
      }
      if (response.err) {
        toast.error(res.err);
      } else {
        ...
        <div className="users_bottom_part">
          <div className="total_updates_top ActiveLinks">
            ...
            <div className="walletAddressHead accountChangeHead ">
              <h5 className="m-0">Add a contributor</h5>
            </div>
                ...
                <form onSubmit={formik.handleSubmit}>
                  ...
                  >
                    ...
                    <div className="col-md-4 col-12">
                      <div className="form-group mb-3">
                        <label htmlFor="online_view_link">Job Role</label>
                        <input
                          type="text"
                          className="form-control"
                          id="job_role"
